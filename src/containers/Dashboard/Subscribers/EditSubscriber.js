...
let EditSubscriber = ({
    ...
    useEffect(() => {
        if (!formData.editSubscriber) return;
        if (!formData.editSubscriber.syncErrors) setNext(true);
        else setNext(false);
      }, [formData]);

    return (
        <div className="update-subscriber">
            <Col md="12">
...
EditSubscriber = reduxForm({
    form: 'editSubscriber', //                 <------ same form name
...
const mapStateToProps = state => {
    const subscriber = state.subscriberReducer.subscriber;
    return {
        initialValues: subscriber,
        formData: state.form
    }
}
export default connect(mapStateToProps)(EditSubscriber);